...
# Função para ler dados do Airtable
def ler_dados():
    ...
    if response.status_code == 200:
        ...
        for r in records:
            f = r.get("fields", {})
            f["id"] = r.get("id")  # Salva o ID interno do Airtable para atualizações
            dados.append(f)
            ...
            if not nome.strip():
                ...
                if ja_votou:
                    ...
                    payload = {"fields": {
                        ...
                        "nome": nome_limpo,
                        "gols_br": str(int(gols_br)),
                        "gols_adv": str(int(gols_adv)),
                    ...
                    st.balloons()
                    st.success(f"✅ {nome_limpo}, palpite salvo! Seu jogador é:
# ==========================================
# ABA 2: RANKINGS
# ==========================================
with aba_ranking:
    ...
    if st.session_state.jogo_atual["status_finalizado"]:
        st.info(f" Placar Final: {st.session_state.jogo_atual['confronto']} ({st.session_state.jogo_atual['placar_real_br']} x {st.session_state.jogo_atual['placar_real_adv']}) \n\n ⚽ Último gol: **{st.session_state.jogo_atual['autor_ultimo_gol']}**")
    else:
    ...
    if not df_nuvem.empty and "jogo" in df_nuvem.columns:
        ...
        if not df_filtrado.empty:
            if st.session_state.jogo_atual["status_finalizado"]:
                ganhadores_premio = df_filtrado[df_filtrado['acertou_placar_exato'].astype(str).str.upper() == "TRUE"]
                if len(ganhadores_premio) != 1:
                    ganhadores_premio = df_filtrado[df_filtrado['ganhou_pelo_jogador'].astype(str).str.upper() == "TRUE"]
                
                if not list(ganhadores_premio.index) == []:
                    st.subheader("🥇 Ganhador(es) do Prêmio da Rodada:")
                    for _, g in ganhadores_premio.iterrows():
                        st.success(f"⭐ **{g['nome']}** venceu!")
        ...
        st.markdown("<h4 style='color: #ffdf00; background-color:#009c3b; padding: 5px 10px; border-radius:4px;'>📈 CLASSIFICAÇÃO GERAL DO TORNEIO</h4>", unsafe_allow_html=True)
...
with aba_admin:
    ...
    if senha == "brasil2026":
            ...
            if not df_calculo.empty:
                ...
                for idx, row in df_calculo.iterrows():
                    if row["jogo"] == st.session_state.jogo_atual["confronto"]:
                        ...
                        up_payload = {"fields": {"pontos": str(pt), "acertou_placar_exato": str(exato)}}
                        requests.patch(f"{URL}/{row['id']}", headers=HEADERS, json=up_payload)
                ...
                st.success("🏆 Pontuações gravadas no Airtable!")
                st.rerun()
        ...
        if st.button("🚨 Resetar Todo o Aplicativo (Zerar Banco)"):
            ...
            st.session_state.jogo_atual["status_finalizado"] = False
            st.success("Banco de dados do Airtable limpo!")
            st.rerun()
